...
This update keeps `aegisub-automation-pack` aligned with the English-facing engineering portfolio while removing dash-line motion from SVG visuals.

## Changes

- Cleaned `assets/aegisub-hero.svg` so SVG motion no longer uses moving dashed or dotted line effects.
- Cleaned `assets/ass-effect-pipeline.svg` so SVG motion no longer uses moving dashed or dotted line effects.
